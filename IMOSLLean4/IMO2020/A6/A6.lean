...
theorem good_add_one : good (· + 1) := λ a b ↦ by
  have h (c : ℤ) : c + (c.natAbs ^ 2 : ℕ) = c * (c + 1) := by
    rw [Int.natCast_pow, Int.natAbs_sq, sq, Int.mul_add, Int.mul_one, add_comm]
...
theorem map_neg_one : f (-1) = 0 := by
  ...
  rw [Int.neg_mul, Int.one_mul, eq_neg_iff_add_eq_zero, ← Int.two_mul, Int.mul_eq_zero] at h
  exact h.resolve_left (ne_of_beq_false rfl)

theorem map_iterate_sq_add_one (a : ℤ) :
    f^[(a + 1).natAbs ^ 2 + 1] a = f^[(a + 1).natAbs ^ 2] (a + 1) := by
  have h0 := h (a + 1) (-1)
  rwa [add_neg_cancel_right, map_neg_one h, Int.mul_zero, add_zero, ← map_iterate_sq h] at h0
...
theorem orbit_zero_bdd_of_not_injective (h0 : ¬f.Injective) :
  ...
  replace h0 : ∃ (a : ℤ) (k : ℕ), 0 < k ∧ f (a + k) = f a := by
    ...
    apply Int.sub_pos_of_lt at h2
    lift y - x to ℕ using h2.le with k hk
    refine ⟨x, k, Int.natAbs_pos.mpr h2.ne.symm, ?_⟩
    rw [hk, add_sub_cancel, h0]
  ...
  replace h0 : ∃ k N, 0 < k ∧ f^[N + k] 0 = f^[N] 0 := by
    ...
    obtain ⟨K, M, h1⟩ : ∃ K M, f^[K] a = f^[M] 0 := by
      obtain h1 | h1 : 0 ≤ -a ∨ 0 ≤ a := (le_total a 0).imp_left (Int.neg_nonneg_of_nonpos)
      · lift -a to ℕ using h1 with m hm
        rcases exists_iter_add_large_eq h a m with ⟨M, h2⟩
        rw [hm, add_neg_self] at h2; exact ⟨M + m, M, h2⟩
...
theorem eq_zero_of_not_injective (h0 : ∃ M, ∀ n, (f^[n] 0).natAbs < M) : f = 0 := by
  ...
  replace h1 (a : ℤ) (ha : a ≠ 0) : f a = 0 := by
    ...
    rwa [Int.natAbs_neg, h3, f.iterate_succ_apply, ← h1, ← f.iterate_succ_apply, ← h3,
      h.map_iterate_sq, Int.neg_mul, eq_neg_iff_add_eq_zero, ← Int.two_mul, Int.mul_eq_zero,
      or_iff_right (ne_of_beq_false (by rfl)), Int.mul_eq_zero, or_iff_right ha] at h2
