...
theorem case2_map_sq_sub_one (h3 : f 0 = -1) (x : R) :
    f (x ^ 2 - 1) = f x ^ 2 - 1 := by
  rw [sq, sq, ← case2_good_alt h h0, sub_self, h3,
    sub_neg_eq_add, add_sub_cancel_right]

/-- (6.4) -/
theorem case2_map_self_mul_add_one_sub_one (x : R) :
    f (x * (x + 1) - 1) = f x * f (x + 1) := by
  rw [← case2_good_alt h h0, sub_add_cancel_left, h0, sub_zero]
...
theorem case2_map_add_two_eq (x : R) :
  ...
  rw [mul_sub, ← add_sub_right_comm, eq_sub_iff_add_eq', add_comm x 2, ← h3]
  refine (Eq.congr ?_ ?_).mp (h3 (-(x + 1)))
  · rw [mul_neg, ← h2, neg_add_eq_sub, neg_sub, two_mul, add_sub_assoc,
      add_sub_cancel_right, add_right_comm, two_mul]
  · rw [h2, add_right_inj, ← sub_eq_add_neg, ← one_add_one_eq_two,
      add_sub_add_right_eq_sub, ← h2, neg_sub]
