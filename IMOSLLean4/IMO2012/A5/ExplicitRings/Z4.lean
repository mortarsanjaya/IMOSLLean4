...
def cast {R : Type _} [AddGroupWithOne R] : ℤ₄ → R
  | ℤ₄0 => 0
  | ℤ₄1 => 1
  | ℤ₄2 => 2
  | ℤ₄3 => -1
...
theorem cast_eq_zero_imp (h : (2 : R) ≠ 0) (x : ℤ₄) (h0 : cast (R := R) x = 0) :
    x = 0 :=
  have h1 : (1 : R) ≠ 0 := λ h1 ↦ h <| by rw [← one_mul (2 : R), h1, zero_mul]
  match x with
    | ℤ₄0 => rfl
    | ℤ₄1 => absurd h0 h1
    | ℤ₄2 => absurd h0 h
    | ℤ₄3 => absurd (neg_eq_zero.mp h0) h1
