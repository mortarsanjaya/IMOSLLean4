...
theorem f_aux_alt1_prod_description :
    ∀ l : List R, f_aux_alt1 l = (l.map M).prod.mulVec (oneZeroVec R)
  | [] => (one_mulVec (oneZeroVec R)).symm
  | r :: l => by rw [map_cons, prod_cons, f_aux_alt1_cons,
      f_aux_alt1_prod_description, mulVec_mulVec]

theorem f_aux_matrix_description1 :
    ∀ l : List R, f_aux l = (f_aux_alt1 l 0, f_aux_alt1 l 0 - f_aux_alt1 l 1)
  | [] => Prod.ext rfl (sub_zero 1).symm
  | r :: l => by
    rw [f_aux_cons, f_aux_alt1_cons, f_aux_matrix_description1,
      Prod.mk.inj_iff, matrix_fin2_mulVec, matrix_fin2_mulVec, M]
...
theorem f_aux_alt2_prod_description :
    ∀ l : List R, f_aux_alt2 l = vecMul (oneZeroVec R) (l.reverse.map M).prod
  | [] => (vecMul_one (oneZeroVec R)).symm
  | r :: l => by rw [f_aux_alt2_cons, reverse_cons, map_append, map_singleton,
      prod_append, prod_singleton, f_aux_alt2_prod_description, vecMul_vecMul]
...
theorem f_aux_matrix_description2 :
  ...
  | r::l => by
    rw [f_aux_cons, f_aux_alt2_cons, f_aux_matrix_description2,
      Prod.mk.inj_iff, matrix_fin2_vecMul, matrix_fin2_vecMul, M]
    simp only [Matrix.head_cons, of_apply, cons_val_one, cons_val_zero]
    refine ⟨?_, ?_⟩
