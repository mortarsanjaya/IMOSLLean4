...
instance [AddCancelCommMonoid G] : AddCancelCommMonoid (ExcellentFun R G) where
  add_left_cancel _ _ _ h := ext λ x ↦ add_left_cancel (ExcellentFun.ext_iff.mp h x)


section

variable [SubtractionCommMonoid G]
...
instance [AddCommGroup G] : AddCommGroup (ExcellentFun R G) :=
