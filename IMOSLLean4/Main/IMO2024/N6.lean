...
The original problem is equivalent to finding all values of $n > 0$
  for which every polynomial over $ℤ/nℤ$ is good.
The official solution implies that this holds for $n > 2$, and "more generally",
  every function over $𝔽ₚ$ is good if $p$ is an odd prime.
(This isn't really more general, as every function from $𝔽ₚ$ to itself is a polynomial.)
We show even more:
* every function from $ℤ/4ℤ$ to itself is good (see below);
* every function from a finite field of characteristic $≠ 2$ to itself is good
  (follows from the official solution).

### The case $n = 4$

Consider an arbitrary function $f : ℤ/4ℤ → ℤ/4ℤ$.
...
theorem good_of_good_polynomial_map [Ring R] [Ring S]
    (φ : R →+* S) (hφ : (⇑φ).Surjective) (P : R[X]) (hP : good (P.map φ).eval) :
    good P.eval := by
...
variable [Ring R] [NoZeroDivisors R] {f : R → R}
...
theorem good_iff_of_domain : good f ↔ ∃ a b c : R,
    ∀ r : R, a * r ^ 2 + b * r + c ≠ 0 ∧ a * r ^ 2 + b * r + c ≠ f r := by
  conv => left; unfold good; right; ext; right; ext; right; ext; ext
          rw [mul_ne_zero_iff, sub_ne_zero, ne_comm (a := f _)]

/-- If `R` is a domain and `f : R → R` does not attain some non-zero value,
  then `f` is good. -/
theorem good_of_map_ne_of_ne_zero (hu : u ≠ 0) (hf : ∀ r, f r ≠ u) : good f := by
  refine good_iff_of_domain.mpr ⟨0, 0, u, λ r ↦ ?_⟩
  rw [zero_mul, zero_mul, add_zero, zero_add]
  exact ⟨hu, (hf r).symm⟩
...
theorem good_of_forall_map_ne_zero [Ring R] [IsDomain R] [DecidableEq R]
  ...
  let g (r : R) : R := if f r = 0 then 1 else f r
  /- Consider the map `g : R → R` with `g(r) = 1` for `f(r) = 0` and `g(r) = f(r)` else.
    Then `g` takes only non-zero values, and so it is good. -/
  obtain ⟨a, b, c, hg⟩ : ∃ a b c, ∀ r,
      a * r ^ 2 + b * r + c ≠ 0 ∧ a * r ^ 2 + b * r + c ≠ g r := by
    refine good_iff_of_domain.mp (hR _ λ r ↦ ?_)
    dsimp only [g]; split_ifs with h
    exacts [one_ne_zero, h]
  ---- Now use `g` to show that `f` is good.
  refine good_iff_of_domain.mpr ⟨a, b, c, λ r ↦ ?_⟩
  set s : R := a * r ^ 2 + b * r + c
  obtain ⟨hr, hr0⟩ : s ≠ 0 ∧ s ≠ g r := hg r
  exact ⟨hr, λ (h2 : s = f r) ↦ hr0 (h2.trans (if_neg (h2.symm.trans_ne hr)).symm)⟩
...
/-- Let `F` be a finite field, and fix two distinct elements `a, b ∈ F`.
  Suppose that every function `f : F → F` with `f(a) = f(b)` and `0 ∉ f(R)` is good.
  Then every function `f : F → F` is good. -/
theorem good_of_forall_map_pair_eq_map_ne_zero
    (hab : a ≠ b) (hF : ∀ f : F → F, f a = f b → (∀ r, f r ≠ 0) → good f) :
    ∀ f : F → F, good f := by
...
/-- Let `F` be a finite field of characteristic `≠ 2`.
  Then every function from `F` to itself is good. -/
theorem good_of_char_ne_two (hF : ringChar F ≠ 2) : ∀ f : F → F, good f := by
  ---- Reduce to the case where `f(-1) = f(1)` and `f` does not attain zero.
  refine good_of_forall_map_pair_eq_map_ne_zero (a := -1) (b := 1)
    (Ring.neg_one_ne_one_of_char_ne_two hF) λ f hf hf0 ↦ ?_
  ---- If `f` does not attain some non-zero value, then we are done.
  by_cases h : ∃ u ≠ 0, ∀ x, f x ≠ u
  · rcases h with ⟨u, hu, hu0⟩
    exact good_of_map_ne_of_ne_zero hu hu0
  ---- Now suppose that `f` attains every non-zero value.
  replace h : ∀ u ≠ 0, ∃ x, f x = u := by
    simpa only [ne_eq, not_exists, not_and, not_forall, Decidable.not_not] using h
  ...
  obtain ⟨y, hy⟩ : ∃ y, f y = (1 - r') * f 0 :=
    h _ (mul_ne_zero (sub_ne_zero.mpr λ hr0 ↦ hr (hr0 ▸ IsSquare.one)) (hf0 0))
  have hr' : r' ≠ 0 := λ hr' ↦ hr (hr' ▸ IsSquare.zero)
  have hy0 : y ≠ 0 := by
    rintro rfl; rw [eq_comm, mul_eq_right₀ (hf0 0), sub_eq_self] at hy
    exact hr' hy
...
/-- If `p` is an odd prime, then every function `f : ZMod p → ZMod p` is good. -/
theorem ZMod_good_of_prime_ne_two (p) [Fact (Nat.Prime p)] (hp : p ≠ 2) :
    ∀ f : ZMod p → ZMod p, good f :=
...
theorem ZMod_good_of_two_lt (hn : 2 < n) (P : (ZMod n)[X]) : good P.eval := by
  ...
  · replace hn : 2 ≤ k := (Nat.pow_lt_pow_iff_right Nat.one_lt_two).mp hn
    replace hn : 4 ∣ 2 ^ k := Nat.pow_dvd_pow 2 hn
...
theorem ZMod_forall_polynomial_good_iff_two_lt (hn : n > 0) :
    (∀ P : (ZMod n)[X], good P.eval) ↔ 2 < n := by
  refine ⟨λ hn0 ↦ Nat.lt_of_not_le λ hn1 ↦ ?_, ZMod_good_of_two_lt⟩
...
theorem final_solution (hn : n > 0) : (∀ P : ℤ[X], natGood n P.eval) ↔ n > 2 := by
  simp_rw [← ZMod_forall_polynomial_good_iff_two_lt hn, natGood_polynomial_iff_good hn]
