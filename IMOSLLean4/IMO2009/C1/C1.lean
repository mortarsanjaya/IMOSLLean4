...
The board consists of $M$ cards in a row,
  one side labelled $0$ and another side labelled $1$.
...
theorem ValidMove_sum_two_pow {X Y : GameState n} (h : X.ValidMove Y) :
    Y.board.sum (λ i ↦ 2 ^ i) < X.board.sum (λ i ↦ 2 ^ i) :=
...
theorem isReachable_central_card_add_numMoves_mod_two
    ...
    (Y.centralCards.card + Y.numMoves) % 2
      = (X.centralCards.card + X.numMoves) % 2 :=
  ReflTransGen.head_induction_on h rfl λ h0 _ h1 ↦ h1.trans <| by
    ---- One move case
    rw [ValidMove_numMoves h0, Nat.add_mod, ValidMove_central_card_mod_two h0,
      ← Nat.add_mod, add_add_add_comm, Nat.add_mod_right]

theorem filter_central_init_card (M n : ℕ) :
    (init M n).centralCards.card = M / n.succ :=
  M.card_multiples n.succ

theorem filter_central_ends {X : GameState n} (h : X.Ends) :
    X.centralCards = ∅ :=
  filter_eq_empty_iff.mpr λ i h0 ↦ Nat.not_dvd_of_pos_of_lt i.succ_pos <|
    Nat.succ_lt_succ <| mem_range.mp <| Ends_iff.mp h h0

/-- The number of moves made has the same parity as
  `⌊M/(n + 1)⌋` when the game ends. -/
theorem numMoves_mod_two_eq_div_of_ends
...
alias final_solution_max_moves := GameState.moves_lt_two_pow

/-- Final solution: Determining the winner -/
alias final_solution_winner := GameState.P1Wins_iff
